...
# testfile stuff
testFile = "index.html"
testFilePath = "/tmp/quic-data/" + testFile
...
# client commands
tcpClientCommand = "wget -O ./tmp/indexX.html http://127.0.0.1/" + \
	testFile + " --no-check-certificate"
#quicClientCommand = "/home/qibao/chromium/src/out/Default/quic_client --host=127.0.0.1 --disable_certificate_verification --allow_unknown_root_cert --port=6121 https://www.example.org/ > ./tmp/download"
quicClientCommand = "/home/qibao/chromium/src/out/Default/quic_client --host=127.0.0.1 --disable_certificate_verification --allow_unknown_root_cert --port=6121 https://www.example.org/ > ./tmp/downloadX"
...
quicChromiumDownloadFilepath = "/home/qibao/Downloads/download"
...
tcpdumpAnalyze = "sudo tcpdump -r ./tmp/test.pcap -tttttnnqv > xxx 2>/dev/null"
...
def main():
	...
	def startCapture(currentProtocol):
	...
	def stopCaptureAndAnalyze(testIndex, params):
		...
		if params.spikedelay > 0:
			...
			print os.path.getsize("./tmp/test.pcap") # reading from relative /tmp/test.pcap
			print os.path.getsize("./tmp/tcpdump.tmp") # reading from relative /tmp/tcpdump.tmp
		...
		#new_tcpdumpAnalyze_1 = tcpdumpAnalyze.replace("xxx", "./raw/"+ str(outputName))
		#new_tcpdumpAnalyze_2 = new_tcpdumpAnalyze_1.replace('X', '_' + params.protocol + '_' + str(i))
		os.system(tcpdumpAnalyze.replace("xxx", "./raw/"+ str(outputName)))
		#os.system(new_tcpdumpAnalyze_2)
		return
	...
	while paramsQueue:
		...
		if params.spikedelay > 0:
			curTests *= 2
		#"main"
		startCapture(params.protocol)
		for i in xrange(curTests):
			#startCapture(i, params.protocol)
			if args.vverbose:
			...
			if params.protocol == "TCP":
				new_tcpClientCommand = tcpClientCommand.replace('X', str(i+1))
				os.system(new_tcpClientCommand)
			else:
				if os.path.isfile(quicChromiumDownloadFilepath):
					os.remove(quicChromiumDownloadFilepath)
				new_quicClientCommand = quicClientCommand.replace('X', str(i+1))
				os.system(new_quicClientCommand)
		...
		stopCaptureAndAnalyze(0,params)
main()
